...
class DWMLParser
{
    ...
    private function _parseXML($xmlData)
    {
        ...
        foreach ($parameters->temperature as $temperature)
        {
			...
			// hourly recorded dew points
            if ($temperature->attributes()->type == 'dew point')
            {
                $hourlyDewPoint = array();
                ...
                for ($i = 0; $i < count($temperature->value); $i++)
                {
                    ...
                    $hourlyDewPoint[$key] = $value;

                    $this->_forecast->setRawWeatherDataForTimestamp($key, 'recorded_dewpoint', $value);
                }

                // save the maximum temperature in the forecast model
                $this->_forecast->setHourlyRecordedDewPoint($hourlyDewPoint);
            }
        ...
        foreach ($parameters->humidity as $humidity)
        {
            ...
            if ($humidity->attributes()->type == 'relative')
            {
